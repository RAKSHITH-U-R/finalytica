...
def dash():
    ...
    goog = yf.download("GOOG", period="1d")
    msft = yf.download("MSFT", period="1d")
    reliance = yf.download("RELIANCE.NS", period="1d")
    hcltech = yf.download("TCS.NS", period="1d")
    infy = yf.download("INFY.NS", period="1d")

    # Get current stock prices
    aapl_price = aapl["Close"]
    goog_price = goog["Close"]
    msft_price = msft["Close"]
    reliance_price = reliance["Close"]
    hcltech_price = hcltech["Close"]
    infy_price = infy["Close"]

    # Get yesterday's closing prices
    aapl_prev_close = aapl["Open"]
    goog_prev_close = goog["Open"]
    msft_prev_close = msft["Open"]
    reliance_prev_close = reliance["Open"]
    hcltech_prev_close = hcltech["Open"]
    infy_prev_close = infy["Open"]

    # Calculate the change in price
    aapl_change = round(aapl_price - aapl_prev_close, 2)
    goog_change = round(goog_price - goog_prev_close, 2)
    msft_change = round(msft_price - msft_prev_close, 2)
    reliance_change = round(reliance_price - reliance_prev_close, 2)
    hcltech_change = round(hcltech_price - hcltech_prev_close, 2)
    infy_change = round(infy_price - infy_prev_close, 2)

    # Change in percentage
    aapl_change_percent = round((aapl_change / aapl_prev_close) * 100, 2)
    goog_change_percent = round((goog_change / goog_prev_close) * 100, 2)
    msft_change_percent = round((msft_change / msft_prev_close) * 100, 2)
    reliance_change_percent = round((reliance_change / reliance_prev_close) * 100, 2)
    hcltech_change_percent = round((hcltech_change / hcltech_prev_close) * 100, 2)
    infy_change_percent = round((infy_change / infy_prev_close) * 100, 2)

    # Prepare data for HTML template
    stock_data = [
        {"Symbol": "AAPL", "Price": aapl_price, "Change": aapl_change, "Change Percent": aapl_change_percent},
        {"Symbol": "GOOG", "Price": goog_price, "Change": goog_change, "Change Percent": goog_change_percent},
        {"Symbol": "MSFT", "Price": msft_price, "Change": msft_change, "Change Percent": msft_change_percent},
        {"Symbol": "RELIANCE.NS", "Price": reliance_price, "Change": reliance_change, "Change Percent": reliance_change_percent},
        {"Symbol": "TCS.NS", "Price": hcltech_price, "Change": hcltech_change, "Change Percent": hcltech_change_percent},
    ]

    return render_template('dash.html', stock_data=stock_data)
...
def get_news(company_name):
    ...
    for i in df.index:
        ...
        date=datetime.fromtimestamp(df['providerPublishTime'][i])
        link=df['link'][i]
